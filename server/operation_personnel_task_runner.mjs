...
async function clearVisiblePersonnelDateRange(page, dialog) {
  ...
  if (!text(await startInput.inputValue()) && !text(await endInput.inputValue())) return startInput;

  const rangePicker = startInput.locator(
    "xpath=ancestor::*[.//input[@placeholder='结束日期']][1]",
  );
  const clear = rangePicker.locator(".ant-calendar-picker-clear");
  const clearCount = await clear.count();
  if (clearCount > 1) throw operationControlError("人员落实日期清除按钮", clearCount);
  if (clearCount === 1) {
    await clear.click({ force: true, timeout: 10_000 });
  } else {
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const start = text(await startInput.inputValue());
    const end = text(await endInput.inputValue());
    if (!start && !end) return startInput;
    if (typeof page.waitForTimeout === "function") await page.waitForTimeout(50);
  }
  throw operationConflict("人员落实日期旧范围未清空，已停止下任务");
}

export async function selectVisiblePersonnelDateRange(page, dialog, start, end) {
  const startInput = await clearVisiblePersonnelDateRange(page, dialog);
