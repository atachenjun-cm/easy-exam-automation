...
test("requirement center renders list and detail surfaces", () => {
  ...
  assert.ok(html.includes("2. 人工审核处理"));
  assert.ok(html.includes("按审核结果选择一个动作"));
  assert.ok(html.includes("需客户补充：信息不完整时使用"));
  assert.ok(html.includes("已审核待客户确认：内容完整但还需客户确认"));
  assert.ok(html.includes("标记人工执行交接：客户确认后进入执行交接"));
...
test("WeChat collector page renders config and scheduler status surfaces", () => {
  ...
  assert.ok(html.includes("每个启用群上线前都需要立即采集本群"));
